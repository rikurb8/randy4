...
describe('End-to-end workflow examples', () => {
  ...
  it('Complete project planning workflow', async () => {
    ...
    const planEntry = allPlans.find(([_, p]) => 
      p.title === plan.title && 
  ...
  it('Research and learning workflow', async () => {
    ...
    const primaryInfo = await informationService.getInformation(primaryInfoRequest);
    ...
    const detailedInfo = await informationService.getInformation(detailedInfoRequest);
    ...
    const learningPlanRequest: PlanRequest = {
      ...
      constraints: ['Study 10 hours per week', 'Focus on practical applications'],
